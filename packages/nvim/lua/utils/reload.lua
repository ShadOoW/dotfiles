...
local modules_to_reload = {"config.options", "config.autocmds", "config.commands", "config.keymaps", "utils.keymap",
                           "utils.file", "utils.string", "lsp.handlers", "plugins"}
...
local function unload_module(module_name)
    ...
    for loaded_module, _ in pairs(package.loaded) do
        if loaded_module:match("^" .. module_name .. "%.") then
            package.loaded[loaded_module] = nil
        end
    end
end
