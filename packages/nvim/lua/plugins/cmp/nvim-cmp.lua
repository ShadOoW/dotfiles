                ...
                format = function(entry, vim_item)
                    ...
                    if kind == 'Method' or kind == 'Function' or kind == 'Constructor' then
                        ...
                        if not params_match or params_match == '' then
                            params_match = label:match('%((.-)%)')
                        end

                        -- If still no parameters, try detail field
                        if not params_match or params_match == '' then
                            if detail and detail ~= '' then
                                params_match = detail:match('%((.-)%)')
                            end
                        end
                        ...
                        if not params_match or params_match == '' then
                            ...
                            if text_edit and text_edit.newText then
                                params_match = text_edit.newText:match('%((.-)%)')
                            end
                        end
                        ...
                        if detail ~= '' then
                            ...
                            local patterns = {'%)%s*:%s*([%w_%.%?<>%[%]]+)', -- "method(params) : ReturnType"
                            ...
                            if return_type ~= '' then
                                ...
                                if return_type:match('%.') then
                                    return_type = return_type:match('([^%.]+)$') or return_type
                                end
                            end
                        end
                    ...
                    elseif kind == 'Field' or kind == 'Property' or kind == 'Variable' or kind == 'Constant' or kind ==
                        'EnumMember' then
