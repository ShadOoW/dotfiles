...
return {
  ...
  config = function()
    ...
    smart_splits.setup({
      ...
      animate = false,
      -- Stop at edge (wrap not supported in Kitty multiplexer)
      at_edge = 'stop',
    ...
    local function safe_swap_buf_left()
      ...
      if left_win ~= current_win then
        ...
        if left_buftype ~= 'neo-tree' then smart_splits.swap_buf_left() end
      else
    ...
    local function safe_swap_buf_right()
      ...
      if right_win ~= current_win then
        ...
        if right_buftype ~= 'neo-tree' then smart_splits.swap_buf_right() end
      else
    ...
    vim.keymap.set('n', '<C-Left>', function() smart_splits.resize_left() end, {
      desc = 'Increase window width',
    })

    vim.keymap.set('n', '<C-Right>', function() smart_splits.resize_right() end, {
      desc = 'Decrease window width',
    })

    vim.keymap.set('n', '<C-Up>', function() smart_splits.resize_up() end, {
      desc = 'Decrease window height',
    })

    vim.keymap.set('n', '<C-Down>', function() smart_splits.resize_down() end, {
    ...
    vim.keymap.set('n', '<leader>w<Left>', function() safe_swap_buf_left() end, {
      desc = 'Swap with left buffer',
    })

    vim.keymap.set('n', '<leader>w<Down>', function() smart_splits.swap_buf_down() end, {
      desc = 'Swap with down buffer',
    })

    vim.keymap.set('n', '<leader>w<Up>', function() smart_splits.swap_buf_up() end, {
      desc = 'Swap with up buffer',
    })

    vim.keymap.set('n', '<leader>w<Right>', function() safe_swap_buf_right() end, {
