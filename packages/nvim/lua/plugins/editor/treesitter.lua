                ...
                if array_initializer then
                    ...
                    if current_index then
                        ...
                        if target_index >= 1 and target_index <= #elements then
                            ...
                            if curr_start_row > target_start_row or
                                (curr_start_row == target_start_row and curr_start_col > target_start_col) then
                                ...
                                first_start_row, first_start_col, first_end_row, first_end_col = curr_start_row,
                                    curr_start_col, curr_end_row, curr_end_col
                                second_start_row, second_start_col, second_end_row, second_end_col = target_start_row,
                                    target_start_col, target_end_row, target_end_col
                                ...
                                first_start_row, first_start_col, first_end_row, first_end_col = target_start_row,
                                    target_start_col, target_end_row, target_end_col
                                second_start_row, second_start_col, second_end_row, second_end_col = curr_start_row,
                                    curr_start_col, curr_end_row, curr_end_col
                                cursor_follows_first = true -- cursor follows to target (first)
                            end

                            -- Perform replacements in order (later position first)
                            vim.api.nvim_buf_set_text(0, first_start_row, first_start_col, first_end_row, first_end_col,
                                vim.split(first_text, '\n'))
                            vim.api.nvim_buf_set_text(0, second_start_row, second_start_col, second_end_row,
                                second_end_col, vim.split(second_text, '\n'))
                            ...
                            if new_cursor_row >= line_count then
                                new_cursor_row = line_count - 1
                            end
                            if new_cursor_row < 0 then
                                new_cursor_row = 0
                            end

                            local line_text =
                                vim.api.nvim_buf_get_lines(0, new_cursor_row, new_cursor_row + 1, false)[1] or ''
                            if new_cursor_col >= #line_text then
                                new_cursor_col = math.max(0, #line_text - 1)
                            end
                            if new_cursor_col < 0 then
                                new_cursor_col = 0
                            end
                    ...
                    if class_body and class_body:type() == 'class_body' then
                        ...
                        if current_index then
                            ...
                            if target_index >= 1 and target_index <= #field_declarations then
                                ...
                                -- Perform replacements in order (later position first)
                                vim.api.nvim_buf_set_text(0, first_start_row, first_start_col, first_end_row,
                                    first_end_col, vim.split(first_text, '\n'))
                                vim.api.nvim_buf_set_text(0, second_start_row, second_start_col, second_end_row,
                                    second_end_col, vim.split(second_text, '\n'))
                                ...
                                local line_text = vim.api.nvim_buf_get_lines(0, new_cursor_row, new_cursor_row + 1,
                                                      false)[1] or ''
                                if new_cursor_col >= #line_text then
                                    new_cursor_col = math.max(0, #line_text - 1)
                                end
