...
return {
	{
		...
		config = function()
			...
			wiki_keymap("c", function()
				...
				if not success then
					vim.notify("WikiJournalCopyToNext not available in current buffer", vim.log.levels.WARN)
				end
			...
			wiki_keymap("l", function()
				...
				if not success then
					vim.notify("Table of Contents not available", vim.log.levels.INFO)
				end
			...
			wiki_keymap("r", function()
				...
				if not success then
					vim.notify("WikiPageRename not available in current buffer", vim.log.levels.WARN)
				end
			end, "Rename Page")
			...
			wiki_keymap("G", function()
				...
				if not success then
					vim.notify("Backlinks not available", vim.log.levels.INFO)
				end
			end, "Find Backlinks")
			...
			wiki_keymap("e", function()
				...
				if not success then
					vim.notify("WikiExport not available", vim.log.levels.INFO)
				end
			end, "Export Wiki")
			...
			wiki_keymap("E", function()
				...
				if not success then
					vim.notify("WikiExportTOC not available", vim.log.levels.INFO)
				end
			end, "Export TOC")
			...
			wiki_keymap("P", function()
				...
				if input and input ~= "" then
					...
					if input ~= "" then
						vim.cmd("WikiOpen projects/" .. input)
					end
				end
				...
				callback = function()
					...
					vim.keymap.set(
						...
						function()
							...
							if not success then
								vim.notify("Checkbox toggle not available", vim.log.levels.WARN)
							end
						end,
						vim.tbl_extend("force", opts, {
