...
return {
    ...
    'nvim-telescope/telescope-project.nvim', 'nvim-telescope/telescope-frecency.nvim',
                    'nvim-telescope/telescope-symbols.nvim', 'nvim-telescope/telescope-file-browser.nvim', -- File browser extension
    ...
    config = function()
        ...
        local copy_selected = function(prompt_bufnr)
            ...
            if entry then
                ...
                if not text_to_copy or text_to_copy == '' then
                    ...
                    -- Ultimate fallback
                    if not text_to_copy then
                        text_to_copy = 'Selected entry'
                    end
                end
            ...
            if #multi_selection > 0 then
                ...
                if entry and (entry.path or entry.filename) then
                    ...
                    vim.schedule(function()
                        -- Use the path as-is from Telescope, don't modify it
                        vim.cmd('badd ' .. vim.fn.fnameescape(file_path))
                        vim.notify('Added file to buffer list: ' .. vim.fn.fnamemodify(file_path, ':t'),
                            vim.log.levels.INFO)
                    end)
                else
                    actions.close(prompt_bufnr)
        ...
        telescope.setup({
            defaults = {
                ...
                file_ignore_patterns = {'%.git/', 'node_modules/', '%.npm/', '__pycache__/', '%.pyc', 'target/',
                                        'build/', 'dist/', '%.o', '%.a', '%.out', '%.class', '%.jar', '%.pdf', '%.mkv',
                                        '%.mp4', '%.zip'},
            ...
            extensions = {
                ...
                frecency = {
                    ...
                    workspaces = {
                        -- Define workspaces for project-scoped frecency
                        ['conf'] = '/home/shad/.config',
                        ['data'] = '/home/shad/.local/share',
                        ['project'] = vim.fn.getcwd -- Dynamic workspace for current project
                    },
                    -- Auto-detect workspace based on current working directory
                    auto_validate = true,
