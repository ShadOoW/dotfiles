...
return {
  ...
  config = function()
    ...
    local open_all_selected = function(prompt_bufnr)
      ...
      -- Debug output
      print('=== DEBUG: open_all_selected ===')
      print('Multi-selection count:', #multi_selection)
      for i, entry in ipairs(multi_selection) do
        print('Entry', i, ':', entry.path or entry.filename or 'no path')
      end
      ...
      if #multi_selection > 0 then
        ...
        for _, entry in ipairs(multi_selection) do
          if entry.path or entry.filename then
            ...
            print('Adding to open list:', file_path)
          end
        end
        ...
        if #opened_files > 0 then
          ...
          vim.schedule(function()
            for _, file_path in ipairs(opened_files) do
              ...
              print('Opening file with absolute path:', abs_path)
              print('Original path was:', file_path)
        ...
        print('No multi-selection, using default')
        actions.select_default(prompt_bufnr)
      end
    end
      ...
      if #multi_selection > 0 then
        ...
        vim.schedule(function()
          for _, file_path in ipairs(added_files) do
            -- Get absolute path to avoid working directory issues
            local abs_path = vim.fn.fnamemodify(file_path, ':p')
            print('Adding to buffer list with absolute path:', abs_path)
            print('Original path was:', file_path)
        ...
        if entry and (entry.path or entry.filename) then
          ...
          vim.schedule(function()
            -- Get absolute path to avoid working directory issues
            local abs_path = vim.fn.fnamemodify(file_path, ':p')
            print('Adding single file to buffer list with absolute path:', abs_path)
            print('Original path was:', file_path)
            vim.cmd('badd ' .. vim.fn.fnameescape(abs_path))
            vim.notify('Added file to buffer list: ' .. vim.fn.fnamemodify(abs_path, ':t'), vim.log.levels.INFO)
          end)
        else
          actions.close(prompt_bufnr)
        end
