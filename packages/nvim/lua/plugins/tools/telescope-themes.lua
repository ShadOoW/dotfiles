  ...
  config = function()
    ...
    local function setup_enhanced_keymaps()
      ...
      vim.keymap.set('n', '<leader>sy', function()
        ...
        if not success then vim.notify('Yank history extension not available', vim.log.levels.WARN) end
      end, {
        desc = 'Yank history',
      })

      vim.keymap.set('n', '<C-f>', function() builtin.live_grep(get_ivy_theme()) end, {
    ...
    -- '\' - Project-scoped recent files finder
    vim.keymap.set('n', '\\', function()
      local cwd = vim.fn.getcwd()
      local builtin = require('telescope.builtin')

      -- Use oldfiles (recent files) filtered by current working directory
      builtin.oldfiles(get_ivy_theme({
        cwd = cwd,
        previewer = false,
        prompt_title = ' 󰈞 Recent Files: ' .. vim.fn.fnamemodify(cwd, ':t'),
        only_cwd = true, -- This is the key - only show files from current working directory
        cwd_only = true, -- Alternative parameter name in some versions
      }))
    end, {
