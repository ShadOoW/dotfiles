...
# If running as root (e.g., via sudo), bypass all sudo checks
if [[ $EUID -eq 0 ]]; then
  export _RUNNING_AS_ROOT="true"
fi
...
declare -A TARGET_DIRS=(
  ...
  ["zsh","rtorrent"]="$HOME"
  ["mimeapps","user-dirs"]="$HOME/.config"
...
# Cache for sudo permission
_SUDO_CACHED=false

# Function to request sudo permission (cached - only asks once)
request_sudo() {
  if [[ "$_SUDO_CACHED" == "true" ]]; then
    return 0
  fi
  # If we're already root (via sudo), skip sudo entirely - no password needed
  if [[ "${_RUNNING_AS_ROOT:-}" == "true" ]]; then
    _SUDO_CACHED=true
    return 0
  fi
...
unstow_all_packages() {
  ...
  for package in "${packages[@]}"; do
    ...
    for target_dir in $target_dirs; do
      ...
      while IFS= read -r -d '' file; do
        ...
        if [[ -L "$target_file" ]]; then
          ...
          if [[ "$link_target" == "$file" || "$link_target" == "$STOW_DIR"* ]]; then
            ...
            # Only request sudo when we actually need it
            if [[ "$use_sudo" == "true" ]]; then
              request_sudo
              sudo rm "$target_file"
            else
      ...
      # Clean up empty directories (only if they exist)
